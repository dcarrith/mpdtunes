...
$("body").on("click","#playpause",function(b){b.preventDefault();$.mobile.hidePageLoadingMsg();$.mobile.loadingMessage="Loading";playing?(updatePlayerDisplay(playing,"pause"),next_track_already_added&&($(inactivePlayerSelector).data("jPlayer").status.srcSet&&$(inactivePlayerSelector).jPlayer("stop"),$(inactivePlayerSelector).jPlayer("volume",1),$(activePlayerSelector).jPlayer("volume",1)),$(activePlayerSelector).jPlayer("pause"),control_mpd("pause"),playing=!1,paused=!0):(updateCurrentTrackInfo(playlist.tracks[track_position]),
updatePlayerDisplay(playing),load_progress_complete=!1,$(activePlayerSelector).jPlayer("setMedia",{mp3:playlist.tracks[track_position].url}),$(activePlayerSelector).jPlayer("play",current_track_position),control_mpd("play"),playing=!0)});$("body").on("click","#next",function(b){b.preventDefault();skipto("next");return!1});$("body").on("click","#prev",function(b){b.preventDefault();skipto("previous");return!1});

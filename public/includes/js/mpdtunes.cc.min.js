...
a),$.mobile.showPageLoadingMsg(theme.bars,"Metadata loaded"))},durationchange:function(a){consoleLog("Inside durationchange event handler for playerOne",a)},playing:function(a){1==primary_player&&(consoleLog("Inside playing event handler",a),$.mobile.showPageLoadingMsg(theme.bars,"Playing"))},timeupdate:function(a){1==primary_player&&(currentTime=a.jPlayer.status.currentTime,totalDuration=a.jPlayer.status.duration,0===playlist.tracks[track_position].url.indexOf("http://")&&(totalDuration="Infinity:NaN"),
...
updateLoadProgressDisplay(seekPercent),100==seekPercent&&(load_progress_complete=!0,updateLoadProgressDisplay(100)))},stalled:function(a){playing&&2==primary_player&&$.mobile.showPageLoadingMsg(theme.bars,"Stalled")},waiting:function(a){2==primary_player&&(consoleLog("Inside waiting event handler",a),$.mobile.showPageLoadingMsg(theme.bars,"Waiting"))},abort:function(a){2==primary_player&&$.mobile.showPageLoadingMsg(theme.bars,"Aborting")},error:function(a){consoleLog("Inside error event handler for playerTwo",
a.jPlayer.error)},warning:function(a){consoleLog("Inside warning event handler for playerTwo",a.jPlayer.warning)},loadstart:function(a){playing&&(2==primary_player&&""!=a.jPlayer.status.src)&&(consoleLog("current track position",current_track_position),0<current_track_position&&(a.jPlayer.status.src==playlist.tracks[track_position].oggurl||a.jPlayer.status.src==playlist.tracks[track_position].url)&&!shuffle_queue?$.mobile.showPageLoadingMsg(theme.bars,"Resuming"):$.mobile.showPageLoadingMsg(theme.bars,
"Loading"))},loadedmetadata:function(a){2==primary_player&&(consoleLog("Inside loadedmetadata event handler",a),$.mobile.showPageLoadingMsg(theme.bars,"Metadata loaded"))},durationchange:function(a){consoleLog("Inside durationchange event handler for playerTwo",a)},playing:function(a){2==primary_player&&(consoleLog("Inside playing event handler",a),$.mobile.showPageLoadingMsg(theme.bars,"Playing"))},timeupdate:function(a){2==primary_player&&(currentTime=a.jPlayer.status.currentTime,totalDuration=
a.jPlayer.status.duration,0===playlist.tracks[track_position].url.indexOf("http://")&&(totalDuration="Infinity:NaN"),playProgress=a.jPlayer.status.currentPercentAbsolute,0==playProgress&&(playProgress=a.jPlayer.status.currentPercentRelative),processTimeUpdate(currentTime,totalDuration,playProgress))},ended:function(a){next_track_already_added?(consoleLog("Updating the currently playing track info",playlist.tracks[track_position]),updateCurrentTrackInfo(playlist.tracks[track_position]),next_track_already_added=
!1,out_volume_left_to_fade=volume_fade-1,in_volume_left_to_fade=volume_fade-1,crossfade_is_in_progress=!1,primary_player=1==primary_player?2:1,consoleLog("Primary player set to "+primary_player),tmpPlayerSelector=activePlayerSelector,activePlayerSelector=inactivePlayerSelector,inactivePlayerSelector=tmpPlayerSelector,consoleLog("Active player selector is now: "+activePlayerSelector),consoleLog("Inactive player selector is now: "+inactivePlayerSelector),$(inactivePlayerSelector).jPlayer("volume",1),
load_progress_complete=!1):repeat_track?skipto("same"):skipto("next")},volumechange:function(a){consoleLog("Player volume set to "+a.jPlayer.options.volume)}});if("undefined"!==typeof playlist&&"undefined"!==typeof playlist.tracks){$("#artistDiv").html(playlist.tracks[track_position].artist);var b=playlist.tracks[track_position].album,b=b.length>max_album_name_string_length?b.substring(0,max_album_name_string_length)+"...":b;$("#albumDiv").html(b).trigger("updatelayout");0===playlist.tracks[track_position].url.indexOf("http://")?
$("#trackDiv").html(playlist.tracks[track_position].url).trigger("updatelayout"):(b=playlist.tracks[track_position].title,b=b.length>max_track_title_string_length?b.substring(0,max_track_title_string_length)+"...":b,$("#trackDiv").html(b).trigger("updatelayout"))}});function control_mpd(b,a){var d="/musicpd/index",c=!1,g=!1,n=!1,h="Loading",f=0;switch(b){case "play":d="/musicpd/control/play";break;case "pause":d="/musicpd/control/pause";break;case "stop":d="/musicpd/control/stop";break;case "next":d="/musicpd/control/next";break;case "prev":d="/musicpd/control/previous";break;case "set_crossfade":d="/musicpd/control/crossfade";break;case "shuffle":d="/musicpd/control/shuffle";break;case "repeat":d="/musicpd/control/repeat";break;case "set_volume":d="/musicpd/control/volume";
...
$(window).on("orientationchange",function(b){setTimeout(function(){$("#playerCurrentlyPlayingDiv").css("max-width",0.9*$(window).width());$("#playerCurrentlyPlayingDiv").width(0.9*$(window).width())},400)});$("html,body").on("taphold",function(b,a){});
